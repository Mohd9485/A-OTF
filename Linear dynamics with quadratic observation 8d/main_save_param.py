...
parameters['ITERATION'] = int(1024*1/16) # 1e4
...
# X_OT , X_prior,Y_prior, OT_param_dict = OTF_param(Y_True,X0,parameters,A,h,t,tau,Noise)
...
# plt.figure()
# plt.hist(X_OT[l,15,num_plot_state,:p],density=True)

# sys.exit()
#%%
# np.savez('./DATA_XX/DATA_file_param_5.npz',\
...
for sim in range(20):    
    ...
    plt.suptitle(sim+1)
    plt.savefig('./DATA_XX_2/sim_'+str(sim+1)+'.png')
    
    # plt.figure()
    # plt.hist(X_OT[l,15,num_plot_state,:p],density=True)
    
    # sys.exit()
    file_name = './DATA_XX_2/DATA_file_param_'+str(sim+1)+'.npz'    
    np.savez(file_name, X_prior = X_prior,Y_prior = Y_prior,OT_param_dict = OT_param_dict)