...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 12 14:06:38 2025
...
def A_OTF_W2(Y,X0_const,parameters,A,h,t,tau,Noise,nearest=True):
...
# =============================================================================
    ...
    #
    avg_dist = np.mean(np.load('distance_W2.npz')['D'])
    # avg_dist = 1
    # avg_dist = np.mean(np.load('distance_between_maps.npz')['D'])
    ...
    Weights = np.zeros((AVG_SIM,N-1,N_maps))
    ...
    for k in range(AVG_SIM):
        ...
        for i in range(N-1):
            ...
            w2_sample = min(J,250)#int(J/5)
            for j in range(N_maps):
                M =  ot.dist(X_prior[j,:w2_sample,:], X1[:w2_sample,:]) 
                
                # Uniform weights if distributions are unweighted
                a = np.ones(w2_sample) / w2_sample # Uniform weights for X
                b = np.ones(w2_sample) / w2_sample # Uniform weights for Y
                
                # Compute the Wasserstein distance (emd2 returns the squared distance)
                distance[j] = np.sqrt(ot.emd2(a, b, M))
            ...
            if nearest:    
                ...
                # weighted map
                lambda_d = 1
                distance = np.exp(-lambda_d*(distance - distance.min()))
                weight = distance/distance.sum()
                map_T = 0
                for j in range(N_maps):
                    MAP_T.load_state_dict(MAP_T_param[j])
                    map_T += weight[j]*MAP_T.forward(X1_test, Y1_true)
            ...
            SAVE_all_X_OT[k,i+1,:,:] = X0
            
            

    print("--- A-OTF time : %s seconds ---" % (time.time() - start_time))
    return SAVE_all_X_OT.transpose(0,1,3,2),Distance,save_index ,time.time()#,  Weights