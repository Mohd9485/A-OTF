...
J = int(1e4) # Number of ensembles EnKF
...
for i in range(len(Num_selected_maps)):
    for j in range(len(t)):
        ...
        w2 = 0
        for k in range(AVG_SIM):
            M =  ot.dist(X_true_particles[k,j,:,:1000].T, X_NEAREST[i][k,j,:,:J_dist].T) 
...
for i in range(len(Num_selected_maps)):
    for j in range(len(t)):
        ...
        w2 = 0
        for k in range(AVG_SIM):
            M =  ot.dist(X_true_particles[k,j,:,:1000].T, X_WEIGHTED_W2[i][k,j,:,:J_dist].T) 
...
for i in range(len(Num_selected_maps)):
    for j in range(len(t)):
        ...
        mmd_sum = 0
        for k in range(AVG_SIM):
            M =  ot.dist(X_true_particles[k,j,:,:1000].T, X_NEAREST_MMD[i][k,j,:,:J_dist].T) 
...
for i in range(len(Num_selected_maps)):
    for j in range(len(t)):
        ...
        mmd_sum = 0
        for k in range(AVG_SIM):
            M =  ot.dist(X_true_particles[k,j,:,:1000].T, X_WEIGHTED_MMD[i][k,j,:,:J_dist].T) 
...
plt.axhline(y=sum(W2_ot)/len(W2_ot),color='m',label='OTF',lw=2,linestyle='--')
...
# plt.title('')
# plt.legend(loc=1,fontsize=20, bbox_to_anchor=(1, 0.8))
...
plt.text(2, 3,textstr ,fontsize=30, bbox = dict(boxstyle='round', facecolor='wheat', alpha=0.5)) 
plt.show()
