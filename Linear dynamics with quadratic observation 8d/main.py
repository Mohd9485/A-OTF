...
for k in Num_selected_maps:
    ...
    # X_OT_nearest,distance,nearest_index,t_nearest,Weights = A_OTF_W2(Y_True,X0,parameters,A,h,t,tau,Noise,nearest=False) 
    ...
    # plt.plot(t[1:],Weights[0,:,:],alpha = 0.7,lw=2)

    # plt.ylabel(r'weight',fontsize=20)
    # plt.xlabel('time',fontsize=20)
    # plt.savefig('./save_figs/XX_weights_n_4.pdf')
...
def A(x,t=0):
    return F @ (x) 
X_true_dist = np.zeros((1,N,L,true_particle))
for i in range(n):
    x = X_True[:,:,2*i:(2*i+2)].reshape(1,N,2)
    y = Y_True[:,:,2*i:(2*i+2)].reshape(1,N,2)
    x0 = np.zeros((1,2,true_particle))
    x0[0] = np.transpose(np.random.multivariate_normal(np.zeros(2),sigmma0*sigmma0 * np.eye(2),true_particle))
    x_true = SIR(y,x0,A,h,t,tau,Noise)
    X_true_dist[0,:,2*i:(2*i+2),:] = x_true.reshape(N,2,true_particle)
