...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 28 13:57:36 2025
...
from SIR import SIR
from OTF_change_mu0 import OTF
...
def Gen_True_Data(L,dy,N,x0_amp,sigmma0,sigmma,gamma,tau,mu_0):
...
parameters['LearningRate'] = 1e-3/1
parameters['ITERATION'] = int(1024/1) #1024
...
for mu_0 in MU_0:
    ...
    for k in range(AVG_SIM):    
        X0_true[k,] = 2*np.transpose(np.random.multivariate_normal(np.zeros(L),sigmma0*sigmma0 * np.eye(L),n_true))
    ...
    for j in range(w2_start_time,len(t)):
        ...
        w2 = 0
        for k in range(AVG_SIM):
            M =  ot.dist(X_true_particles[k,j,:,:1000].T, X_OT_nearest[k,j,:].T) 
...
plt.xlabel(r"$\mu_{0}$",fontsize=20)
