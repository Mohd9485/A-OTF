...
from SIR import SIR
from OTF_change_sigma0 import OTF
...
parameters['Final_Number_ITERATION'] = int(64/4) #int(64*2) #ITERATION 
...
W2_amortized = []
for sig in SIGMA_0:
    sigmma0 = sig # Noise in the initial state distribution
    ...
    # X_OT =  np.load('./particles_run/X_OT_particles_5_sims_5886.npz')['X_OT']
    # time_OT =  np.load('./particles_run/X_OT_particles_5_sims_5886.npz')['time_OT']
    # X_True =  np.load('./particles_run/X_OT_particles_5_sims_5886.npz')['X_True']
    # Y_True =  np.load('./particles_run/X_OT_particles_5_sims_5886.npz')['Y_True']
    # X0 =  np.load('./particles_run/X_OT_particles_5_sims_5886.npz')['X0']
    ...
    parameters['NUM_NEURON'] =  int(64/1) #16
    ...
    #         plt.plot(t,X_OT[l,:,num_plot_state,:1000],'b',ls='none',marker='o',ms=4,alpha = 0.1)
    ...
    for j in range(w2_start_time,len(t)):
        ...
        w2 = 0
        for k in range(AVG_SIM):
            M =  ot.dist(X_true_particles[0,j,:,:1000].T, X_OT_nearest[0,j,:].T) 
    ...
    for j in range(w2_start_time,len(t)):
        ...
        w2_s = 0
        for k in range(AVG_SIM):
            M =  ot.dist(X_true_particles[0,j,:,:1000].T, X_EnKF[0,j,:].T) 
            ...
            M =  ot.dist(X_true_particles[0,j,:,:1000].T, X_SIR[0,j,:].T) 
...
plt.plot(SIGMA_0,W2_enkf,'v--',color='g',label='EnKF',lw=2)
plt.plot(SIGMA_0,W2_sir,'s--',color='b',label='SIR',lw=2)
plt.plot(SIGMA_0,W2_ot,'o--',color='m',label='OTF',lw=2)

plt.plot(SIGMA_0,W2_amortized,'D-',lw=2,label=r'$OT_{Amortized}$')
